...
class SafeRepairAuditDetails:
    ...
    def __post_init__(self) -> None:
        ...
        _sha256(self.recommendation_sha256, "safe_repair_audit_recommendation_digest_invalid")
        _sha256(self.effect_receipt_sha256, "safe_repair_audit_effect_receipt_invalid", optional=True)
        _sha256(self.post_condition_evidence_sha256, "safe_repair_audit_post_condition_evidence_invalid", optional=True)
        if type(self.resource_generation) is not int or self.resource_generation < 0:
            raise SafeRepairAuditEvidenceError("safe_repair_audit_resource_generation_invalid")
        ...
        if type(self.recovery_proven) is not bool:
            raise SafeRepairAuditEvidenceError("safe_repair_audit_recovery_proven_invalid")
        if type(self.post_condition_verified) is not bool:
            raise SafeRepairAuditEvidenceError("safe_repair_audit_post_condition_verified_invalid")
        if self.job_state is RepairJobState.SUCCEEDED:
            if not self.post_condition_verified or self.post_condition_evidence_sha256 is None or self.effect_receipt_sha256 is None:
                raise SafeRepairAuditEvidenceError("safe_repair_audit_false_success_rejected")
        elif self.post_condition_verified:
            raise SafeRepairAuditEvidenceError("safe_repair_audit_verified_state_invalid")
...
def safe_repair_audit_details(*, job: SafeAutoRepairJob, recommendation: SafeAutoRepairRecommendation) -> SafeRepairAuditDetails:
    ...
    recommendation_id = _identifier(recommendation.recommendation_id, "safe_repair_audit_recommendation_id_invalid")
    job_recommendation_id = _identifier(job.recommendation_id, "safe_repair_audit_job_recommendation_id_invalid")
    if recommendation_id != job_recommendation_id:
        raise SafeRepairAuditEvidenceError("safe_repair_audit_recommendation_mismatch")

    recommendation_sha256 = _recommendation_digest(recommendation)
    if job.recommendation_sha256 != recommendation_sha256:
        raise SafeRepairAuditEvidenceError("safe_repair_audit_recommendation_digest_mismatch")
