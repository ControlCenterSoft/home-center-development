...
def _validate_identity(evidence: IdentityProviderQualificationEvidence) -> None:
    ...
    bool_fields = tuple(
        ...
        for name in (
            ...
            "secret_reference_supported", "real_provider_exercised", "real_target_exercised",
            "account_absence_preflight_exercised", "create_contract_validated",
            "readback_contract_validated", "secret_reference_only",
            "secret_values_absent_from_evidence", "one_shot_mutation_preserved",
            "ambiguous_outcome_fail_closed", "exact_operation_binding_verified",
            "account_identity_verified", "home_directory_verified", "profile_verified",
            "emergency_admin_isolated", "arbitrary_privilege_grant_forbidden",
            "unrelated_account_mutation_forbidden", "external_publication_forbidden",
            "portable_home_exercised", "portable_profile_exercised",
        )
    )
...
def evaluate_identity_provider_qualification(evidence: IdentityProviderQualificationEvidence) -> IdentityProviderQualificationDecision:
    ...
    if blockers:
        ...
        evidence_sha256 = hashlib.sha256(json.dumps(canonical, sort_keys=True, separators=(",", ":"), ensure_ascii=True).encode("ascii")).hexdigest()
