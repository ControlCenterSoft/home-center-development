"""Canonical migration payload preparation for Home Center 0.64 recommendation history.

This module defines the append-only schema delta that must be wired into StateStore as
migration 5 by the qualification/integration lane. Keeping the SQL in one product module
prevents the history repository from silently self-migrating and gives release checks an
exact immutable payload to compare before integration.
"""
...
def normalized_migration_sql() -> str:
    """Return deterministic SQL text for release/migration identity checks."""
