...
class RoleIdentityProvisioningApiRuntimeService:
    ...
    def _validate_adapter(
        ...
        if (
            ...
            raise IdentityProvisioningApiRuntimeError(
                "identity_api_provider_registration_invalid"
            )
    ...
    def register_provider(
        ...
        with self._lock:
            ...
            # The durable execution runtime receives the same exact adapter only
            # after qualification metadata has been checked here.
            try:
                self.execution.register_adapter(capability.provider_id, adapter)
            except IdentityProvisioningRuntimeError as exc:
                raise IdentityProvisioningApiRuntimeError(exc.code) from exc
            self._providers[capability.provider_id] = _ProviderRegistration(
        ...
        *,
        ...
        try:
            plan = build_role_identity_provisioning_plan(
                ...
                provider=registration.capability,
                member_id=member_id,
                account_name=account_name,  # normalized by the domain contract
    ...
    def _observe_preflight(
        ...
        *,
        ...
        try:
            ...
            return evaluate_account_preflight(
                plan=plan,
                provider=registration.capability,
    ...
    def preflight(
        ...
        *,
        ...
        decision, _observation = self._observe_preflight(
    ...
    def execute(
        ...
        *,
        ...
        plan = self._load_plan(actor, plan_id)
        registration = self._revalidate_plan(actor, plan)
        decision, observation = self._observe_preflight(
            registration=registration,
            plan=plan,
        )
