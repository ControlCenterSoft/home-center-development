...
Provider capability and preflight evidence are never accepted from the client.
The client may select an already-qualified provider ID, review a server-generated
plan, request a read-only preflight, and explicitly confirm execution using opaque
``secret://`` references. The runtime revalidates Household authority, exact plan,
provider qualification and a fresh server-side account-absence preflight before
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path in self.IDENTITY_PLAN_POSTS:
                ...
                request = self._schema_body(body, schema=PREFLIGHT_REQUEST_SCHEMA, fields={"plan_id"})
            ...
            forbidden = {"household_actor_not_bound", "household_member_disabled", "identity_api_not_authorized"}
            not_found = {"household_not_configured", "identity_api_plan_not_found"}
            conflict = {
                "identity_api_plan_conflict", "identity_api_plan_stale",
                "identity_api_confirmation_required", "identity_api_preflight_not_ready",
                "identity_runtime_idempotency_conflict", "identity_runtime_execution_in_progress",
                "identity_runtime_previous_attempt_failed", "identity_runtime_job_state_invalid",
            }
            unavailable = {
                "identity_api_provider_unavailable", "identity_api_preflight_unavailable",
                "identity_runtime_adapter_unavailable", "identity_runtime_provider_timeout",
                "identity_runtime_provider_error", "identity_runtime_verification_unavailable",
            }
            ...
            self._error(HTTPStatus.BAD_REQUEST, code, "Некорректный запрос управления учётной записью", correlation_id)
