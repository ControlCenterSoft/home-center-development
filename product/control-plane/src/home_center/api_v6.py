...
This boundary intentionally exposes only read-only Actual State reconciliation.  It
inherits the existing authentication, first-login, external-access and same-origin
fences, requires an explicit HTTP idempotency key, and grants no Desired State,
backend mutation, infrastructure mutation or external-publication authority.
"""
...
class RuntimeRequestHandlerV6(RuntimeRequestHandlerV5):
    """Expose 0.59 read-only policy reconciliation behind existing web fences."""
