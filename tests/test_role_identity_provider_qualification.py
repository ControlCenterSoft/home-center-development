...
def test_complete_real_provider_evidence_materializes_trusted_capability_without_authority() -> None:
    ...
    assert decision.adapter_artifact_sha256 == DIGEST
...
def test_decision_validates_against_closed_public_schema() -> None:
    schema = json.loads(
        (ROOT / "contracts/household/role-identity-provider-qualification.v1.schema.json").read_text(encoding="utf-8")
    )
    jsonschema.Draft202012Validator(schema).validate(evaluate_identity_provider_qualification(_evidence()).to_dict())


def test_invalid_candidate_identity_is_rejected_before_evaluation() -> None:
    with pytest.raises(IdentityProviderQualificationError, match="identity_provider_qualification_revision_invalid"):
