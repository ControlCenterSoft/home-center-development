...
class SafeRepairAuditEvidenceTests(unittest.TestCase):
    def test_projection_binds_exact_success_and_exposes_no_secret_authority(self) -> None:
        ...
        payload = safe_repair_audit_details(job=job, recommendation=recommendation).to_dict()
        ...
        for key in (
            ...
            "generic_infrastructure_mutation_authorized",
            "external_publication_authorized",
            "automatic_retry_authorized",
        ):
            self.assertFalse(payload[key])
        for key in ("idempotency_key", "command", "credential", "provider_payload"):
    ...
    def test_projection_rejects_cross_bound_recommendation(self) -> None:
        ...
        other = evaluate_safe_auto_repair(candidate=other_candidate, policy=other_policy)
        with self.assertRaisesRegex(SafeRepairAuditEvidenceError, "safe_repair_audit_recommendation_mismatch"):
    ...
    def test_projection_rejects_tampered_recommendation_digest(self) -> None:
        ...
        with self.assertRaisesRegex(SafeRepairAuditEvidenceError, "safe_repair_audit_recommendation_digest_mismatch"):
    ...
    def test_projection_digest_matches_canonical_recommendation_payload(self) -> None:
        recommendation, job = _fixture()
        expected = hashlib.sha256(canonical_json(recommendation.to_dict()).encode("utf-8")).hexdigest()
        details = safe_repair_audit_details(job=job, recommendation=recommendation)
