...
from home_center.operation_recovery_retry_verification_receipt import (
    OperationRecoveryRetryVerificationObservation,
    OperationRecoveryRetryVerificationReceiptCoordinator,
...
class OperationRecoveryRetryVerificationReceiptTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.request = OperationRecoveryRetryRequest(
            ...
            correlation_id="correlation-recovery-retry-verify-001",
        ...
        command_request = OperationCommandRequest(
            ...
            idempotency_key="restart-operation-retry-verify-001",
            target_node_id="home-node-a",
            service="home-center.service",
            reason="recover supervised service",
            correlation_id="correlation-original-verify-001",
        )
        ...
        execution_coordinator = OperationRecoveryRetryExecutionReceiptCoordinator(
            self.store
        )
        with patch(
            ...
            self.execution, changed = execution_coordinator.record(
                self.claim.claim_id,
                self.admission,
                self.prior_verification,
                self.prior_execution,
        ...
        self.coordinator = OperationRecoveryRetryVerificationReceiptCoordinator(
            self.store
        )
    ...
    @staticmethod
    def _active_observation() -> OperationRecoveryRetryVerificationObservation:
        return OperationRecoveryRetryVerificationObservation(
            True,
            False,
            0,
            45,
    ...
    def test_success_closes_retry_as_rolled_back_without_authority(self) -> None:
        before_audit = len(self.store.audit_events())
        receipt, changed = self._record(self._active_observation())
        ...
        self.assertEqual(self.execution.to_state_version, receipt.from_state_version)
        self.assertEqual(
            self.execution.to_state_version + 1,
        ...
        current = self.store.operation_job(self.execution.job_id)
        assert current is not None
        self.assertEqual("rolled_back", current["state"])
        self.assertFalse(current["recovery_required"])
        self.assertEqual(
            receipt.to_dict(),
            current["evidence"]["recovery_retry_verification_receipt"],
        )
        self.assertEqual(
            receipt.receipt_id,
            current["recovery"]["verification"]["receipt_id"],
        )
        self.assertEqual(before_audit + 1, len(self.store.audit_events()))
        self.store.verify_audit_chain()

    def test_exact_replay_is_idempotent_without_second_transition(self) -> None:
    ...
    def test_conflicting_replay_observation_is_rejected(self) -> None:
        self._record(self._active_observation())
    ...
    def test_wrong_state_fails_closed_and_requires_recovery(self) -> None:
    ...
    def test_worker_rebind_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            OperationRecoveryRetryVerificationReceiptError,
    ...
    def test_recovery_contract_drift_is_rejected(self) -> None:
        ...
        with self.store._lock, self.store._connection:
            self.store._connection.execute(
                ...
                (canonical_json(recovery), self.execution.job_id),
            )
    ...
    def test_final_revalidation_rejects_evidence_drift(self) -> None:
        receipt, _ = self._record(self._active_observation())
        revalidate_operation_recovery_retry_verification_receipt(
            self.store,
            receipt,
            self.execution,
        ...
        with self.store._lock, self.store._connection:
            self.store._connection.execute(
                ...
                (canonical_json(evidence), self.execution.job_id),
            )
        with self.assertRaisesRegex(
            OperationRecoveryRetryVerificationReceiptError,
    ...
    def test_observation_rejects_unbounded_or_inconsistent_values(self) -> None:
        with self.assertRaisesRegex(
            OperationRecoveryRetryVerificationReceiptError,
            "elapsed_ms",
        ):
