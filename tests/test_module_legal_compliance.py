...
class ModuleLegalComplianceTests(unittest.TestCase):
    ...
    def test_authority_tampering_is_rejected(self) -> None:
        tampered = replace(_build(), execution_authorized=True)
        with self.assertRaisesRegex(ModuleLegalComplianceError, "legal_compliance_evidence_rejected"):
    ...
    def test_schema_matches_runtime_and_is_closed(self) -> None:
        schema_path = Path(__file__).parents[1] / "contracts/modules/module-legal-compliance-evidence.v1.schema.json"
        ...
        for name in ("admission_authorized", "installation_authorized", "execution_authorized", "production_mutation_enabled", "external_publication_authorized"):
