...
def test_repair_migrations_are_installed_once_in_canonical_order() -> None:
    ...
    installed_history = [
        ...
        if version == SAFE_AUTO_REPAIR_HISTORY_MIGRATION_VERSION
    ]
    installed_jobs = [
        sql for version, sql in MIGRATIONS
        if version == SAFE_AUTO_REPAIR_JOB_MIGRATION_VERSION
    ]
    assert len(installed_history) == 1
    assert len(installed_jobs) == 1
    assert _normalize(installed_history[0]) == normalized_migration_sql()
    assert _normalize(installed_jobs[0]) == normalized_job_migration_sql()


def test_installed_migrations_match_repository_schemas_exactly() -> None:
    assert normalized_migration_sql() == _normalize(
        SQLiteSafeAutoRepairHistoryRepository.schema_sql()
    )
    assert normalized_job_migration_sql() == _normalize(
        SQLiteSafeAutoRepairJobRepository.schema_sql()
    )


def test_prepared_migrations_are_idempotent_and_create_only_bounded_schema() -> None:
    ...
    try:
        ...
        history_columns = {
            ...
            for row in connection.execute(
                "PRAGMA table_info(safe_auto_repair_recommendations)"
            ).fetchall()
        }
        ...
        job_columns = {
            row[1]: row[2]
            for row in connection.execute(
                "PRAGMA table_info(safe_auto_repair_jobs)"
            ).fetchall()
        }
        assert job_columns == {
            "job_id": "TEXT",
