...
def test_admission_same_idempotency_key_cannot_be_rebound_to_other_effect(tmp_path) -> None:
    household_store = _household_store()
    snapshot = household_store.read("home-main")
    ...
    try:
        ...
        # Different typed job action has its own idempotency namespace, so both can coexist safely.
        other = service.admit(
            actor="parent-session",
