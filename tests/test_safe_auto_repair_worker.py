...
from home_center.safe_auto_repair_adapter import PostConditionState, SafeRepairAdapterRegistry, SafeRepairAdapterResult, SafeRepairPostConditionObservation
...
class Adapter:
    ...
    def __init__(self, *, outcome=RepairExecutionOutcome.ACCEPTED, readback=PostConditionState.MATCHED, fail_execute=False):
    ...
    def execute(self, request):
        ...
        return SafeRepairAdapterResult(job_id=request.job_id, recommendation_id=request.recommendation_id, action=request.action, outcome=self.outcome, effect_receipt_sha256="c" * 64)

    def read_back(self, request):
        self.read_calls += 1
        return SafeRepairPostConditionObservation(job_id=request.job_id, recommendation_id=request.recommendation_id, household_id=request.household_id, resource_id=request.resource_id, observed_generation=request.resource_generation, observation_sha256="d" * 64, state=self.readback)


def setup(outcome=RepairExecutionOutcome.ACCEPTED, readback=PostConditionState.MATCHED, fail_execute=False):
    candidate = RepairCandidate(household_id="household-1", resource_id="derived-index-1", resource_generation=1, evidence_sha256="a" * 64, action=RepairAction.REBUILD_DERIVED_INDEX, risk=RepairRisk.LOW, recovery_proven=True, post_condition_verifiable=True)
    ...
    admission = evaluate_safe_auto_repair_admission(reviewed=recommendation, current_candidate=candidate, current_policy=policy)
    job = build_safe_repair_job(admission=admission, idempotency_key="repair-worker-test-001", created_at_epoch=100)
    repository = Repository(job)
    adapter = Adapter(outcome=outcome, readback=readback, fail_execute=fail_execute)
...
def test_worker_executes_once_then_requires_matched_readback_for_success() -> None:
    worker, repository, adapter, recommendation = setup()
    completed = worker.run(job_id=repository.job.job_id, recommendation=recommendation, now_epoch=110)
    ...
    replay = worker.run(job_id=repository.job.job_id, recommendation=recommendation, now_epoch=120)
    assert replay.state is RepairJobState.SUCCEEDED
    assert adapter.execute_calls == 1
    assert adapter.read_calls == 1


def test_crash_after_running_persistence_never_reinvokes_adapter() -> None:
...
def test_verifying_restart_resumes_readback_without_second_execution() -> None:
    worker, repository, adapter, recommendation = setup()
    request_job = repository.job
    from home_center.safe_auto_repair_job import record_safe_repair_execution, start_safe_repair_job
    running = start_safe_repair_job(request_job, updated_at_epoch=110)
    repository.job = record_safe_repair_execution(running, outcome=RepairExecutionOutcome.ACCEPTED, effect_receipt_sha256="c" * 64, updated_at_epoch=111)
    completed = worker.run(job_id=repository.job.job_id, recommendation=recommendation, now_epoch=120)
    assert completed.state is RepairJobState.SUCCEEDED
    assert adapter.execute_calls == 0
    assert adapter.read_calls == 1


def test_unknown_readback_fails_closed() -> None:
