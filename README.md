...
- Текущий release candidate после 0.40.0: **0.41.0**; до официальной публикации он остаётся предварительным.
- Возможности более поздних подготовленных development-линий не считаются текущим release candidate и не должны описываться как уже доступные пользователю.

Home Center 0.41.0 добавляет evidence-only exact-state revalidation совместимости, введённой в 0.40.0. Повторно проверяются исходный module admission binding, Home Service profile и required service contracts; неизменившееся evidence получает результат `current`, drift — `stale`, а повреждённые или несогласованные данные отклоняются fail-closed. Эта возможность не разрешает admission, installation, execution, production mutation или external publication.
