...
Текущий кандидат `0.23.0` сохраняет durable home-service transition scope. Уже подготовленные кандидаты `0.24.0` и `0.25.0` продолжают эту же transition/evidence линию, поэтому Household/Intent foundation и последующие этапы «Уютного» не закрепляются за уже занятыми номерами версий до появления соответствующего фактического состава кода.
