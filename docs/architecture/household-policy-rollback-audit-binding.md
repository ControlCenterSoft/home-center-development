...
Для recovery после неоднозначного прерывания проверяется exact Audit event `household.policy.desired-state.rollback.recover` с outcome `finalized-existing-write`, тем же actor/resource/rollback id, фактической generation/bundle и history evidence.

Idempotent replay `already-rolled-back` не создаёт новое доказательство: он обязан повторно пройти проверку исходного exact completion/recovery Audit event.
