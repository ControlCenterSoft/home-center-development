...
Опубликованный исходный релиз Home Center — `0.23.0`. Он завершает durable service-transition scope и не включает полноценный «Уютный» UI.

Фактическая кандидатная линия после него уже имеет собственный подтверждённый состав:
...
- `0.29.0` — compatibility admission для verified Module Manifest v2 artifacts;
- `0.30.0` — exact-state revalidation module-admission decision;
...
- `0.33.0` — детерминированная cross-candidate compatibility evaluation для ограниченного набора module candidates на одном точном runtime compatibility snapshot.

Версии `0.24.0`–`0.33.0` остаются предварительными до отдельной официальной публикации. Compatibility/admission evidence не разрешает installation, execution, production mutation или external publication.

Полноценный mobile-first UI «Домой»/«Семья»/«Мой дом», role-driven provisioning, рекомендации, QR-гости и безопасный auto-repair развиваются по фактическому составу последующих версий; их нельзя закреплять за уже занятыми номерами без соответствующей реализации.

Каждая новая пользовательская capability должна определить Full/Core representation, Household/Intent representation либо явное N/A, mapping intent/policy → Desired State, permissions/safety gates, post-condition verification и recovery behavior.
